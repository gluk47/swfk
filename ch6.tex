...
When Python is installed on your computer, a whole pile of functions and modules are also installed. Some functions are available by default.  \code{range} is a function we've already seen. \code{file}\index{functions!file} is another function we haven't use yet.  To see how \code{file} is used:

\subsection*{\color{BrickRed}If you're using Windows$\ldots$}\index{Windows}

Open Notepad and type a few words, then save the file onto your C drive, by:
...
Open a text editor, type a few words and then save the file to your home directory, by clicking File, Save, selecting `Home Folder' (or `Home Directory' or `Home') and typing `test.txt' in the file name box (see figure~\ref{fig19} for an example).
...
\subsection*{\color{BrickRed}Continuing from here$\ldots$}
